...
import { formatDate, initials } from "@/lib/projects";
    ...
    <div className="table-card">
      ...
      {items.map((p) => (
        <Link className="project-row" href={`/project/${p.slug}`} key={p.id}>
          <div className="project-cell"><div className="project-icon">{initials(p.name)}</div><div><strong>{p.name}</strong><small>{p.description.slice(0, 34)}...</small></div></div>
          <div className="muted">{p.chain}</div>
          <div><span className="type-pill">{p.event}</span></div>
          <div className="muted">TBA</div>
