...
if (-not (Test-Path $path)) {
    Write-Host "File not found: $path (run this from the project root)" -ForegroundColor Red
    exit 1
}

Copy-Item $path "$path.bak" -Force
Write-Host "Backup created: $path.bak" -ForegroundColor Yellow
...
if ($pattern.IsMatch($content)) {
    ...
    Write-Host "SUCCESS: DATE block replaced" -ForegroundColor Green
} else {
    Write-Host "FAILED: pattern not found, DATE block not replaced" -ForegroundColor Red
    Write-Host "Run this to inspect: Select-String -Path $path -Pattern DATE -Context 0,60" -ForegroundColor Yellow
    exit 1
}
